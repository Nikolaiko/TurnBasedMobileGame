...
import 'package:turn_based_game/network/dto/user_data/user_login_data_serializer.dart';
import 'package:turn_based_game/network/dto/user_data/user_profile_dto.dart';
import 'package:turn_based_game/network/dto/user_data/user_profile_serializer.dart';
...
class UserRepository {
  ...
  Future<NetworkResponse<UserProfile>> tryRegisterUser(String name, String password) async {
    final loginData = UserLoginDataDto(
      (builder) => builder
        ..name = name
        ..password = password
    ...
    try {            
      var response = await http.post(
        NetworkConstants.user_register_url,
        body: loginDataSerializers.serializeWith(UserLoginDataDto.serializer, loginData)
      );
